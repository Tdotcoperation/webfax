  ...
  constructor(){
    ...
    this.port.onmessage=e=>{
      ...
      if(m.type==='start'){
        ...
        this.leaderFrames=Math.round(sampleRate*.9);
        this.gapFrames=Math.round(sampleRate*.30);
        this.tailFrames=Math.round(sampleRate*.22);
        this.totalDibits=this.bytes.length*4;
        this.active=true; this.stage='leader'; this.pos=0; this.byteIndex=0; this.dibitIndex=0; this.phase=0;
      } else if(m.type==='stop'){ this.active=false; this.stage='idle'; }
    };
  }
  tone(freq){
    this.phase += 2*Math.PI*freq/sampleRate;
    if(this.phase>2*Math.PI)this.phase-=2*Math.PI;
    return Math.sin(this.phase)*0.22;
  }
  process(inputs,outputs){
    ...
    for(let i=0;i<out.length;i++){
      ...
      if(this.stage==='leader'){
        out[i]=this.tone(this.leaderFreq);
        ...
        out[i]=this.tone(this.freqs[dibit]);
        if(++this.pos>=this.symbolFrames){
          this.pos=0; this.phase=0; this.dibitIndex++;
          if(this.dibitIndex===4){this.dibitIndex=0;this.byteIndex++; if(this.byteIndex%8===0)this.port.postMessage({type:'progress',done:this.byteIndex,total:this.bytes.length});}
          if(this.byteIndex>=this.bytes.length){this.stage='tail';this.pos=0;}
        }
      }else if(this.stage==='tail'){
        out[i]=0;
        if(++this.pos>=this.tailFrames){
          this.active=false;this.stage='idle';this.port.postMessage({type:'done'});
        }
      }
    }
    return true;
...
class ModemCapture extends AudioWorkletProcessor {
  constructor(){
    super(); this.buf=new Float32Array(512); this.pos=0;
  }
  process(inputs){
    ...
    for(let i=0;i<input.length;i++){
      ...
      if(this.pos===this.buf.length){
        const send=this.buf;
        this.buf=new Float32Array(512);this.pos=0;
